# Doctor — the iNFT monorepo

**Doctor is an iNFT**: an autonomous AI agent fused with an NFT — whoever holds the token holds
the agent. This repository is its **body**. Underneath the name **Doctor** runs a complete
**Pi coding agent** (the substrate); the **Doctor neural soul** is the identity.

> Forged from the global genesis template **[inft-i01](https://github.com/devclone20/inft-i01)**.
> The template is the mold; **Doctor is a real, named instance** of the CLONE FRAME line.

## Three names, one identity
**Doctor** (its name) · **iNFT** (its species) · **Pi** (its substrate).

## Run it
```bash
bash scripts/setup.sh              # install the Pi substrate (pinned, no sudo)
pi                                 # then /login to connect YOUR model key (BYOK)
bash scripts/boot.sh               # boot Doctor with its soul + skills (pi -a)
bash scripts/install-command.sh    # then type `doctor` in the CLONE FRAME iT terminal
```

## No wallet by design

Doctor takes no economic action and holds no funds — its value is its craft, not a wallet (see `soul/neural_soul.md`).
